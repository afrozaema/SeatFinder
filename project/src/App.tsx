...
function App() {
  ...
  // Load centers data on component mount
  useEffect(() => {
    loadCentersData();
    fetchCurrentTime();
    fetchQuote();
  ...
  // Ema Advice updates every 2 seconds when student data is available
  useEffect(() => {
    if (studentData) {
    ...
    if (studentData) {
      const interval = setInterval(() => {
        ...
        if (diff > 0) {
          ...
          setExamCountdown(`${hours}h ${minutes}m ${seconds}s`);
        } else {
  ...
  const fetchCurrentTime = async () => {
    try {
      ...
      setCurrentTime(dateTime.toLocaleTimeString('en-US', {
        hour: '2-digit',
        minute: '2-digit',
        hour12: true
      }));
      
      setCurrentDate(dateTime.toLocaleDateString('en-US', {
        weekday: 'long',
        year: 'numeric',
        month: 'long',
        day: 'numeric'
  ...
  const fetchWeather = async () => {
    try {
      // Using a demo weather API call - in production, you'd need an API key
  ...
  const calculateArrivalTime = (reportTime: string) => {
    const [time, period] = reportTime.split(' ');
    const [hours, minutes] = time.split(':').map(Number);
    ...
    reportDate.setHours(hour24, minutes, 0, 0);
    
    const arrivalDate = new Date(reportDate.getTime() - 30 * 60000); // 30 minutes earlier
  ...
  const extractCoordinates = (mapUrl: string) => {
    ...
    // Fallback coordinates (Dhaka, Bangladesh)
  ...
  return (
    <div className={`min-h-screen transition-all duration-500 floating-dots ${
      ...
      {/* Enhanced Header with Live Status - Always Visible */}
      <div className={`shadow-lg border-b backdrop-blur-md ${
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-3 sm:py-4">
          {/* Status Bar - Always Visible on All Devices */}
          <div className="flex items-center justify-between mb-3 text-sm">
            <div className="flex items-center gap-3 sm:gap-4">
              <div className={`flex items-center space-x-1 ${isOnline ? 'text-green-600' : 'text-red-600'}`}>
                {isOnline ? <Wifi className="w-3 h-3 sm:w-4 sm:h-4" /> : <Wifi className="w-3 h-3 sm:w-4 sm:h-4 opacity-50" />}
      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 py-4 sm:py-6">
        {/* Compact Hero Section - WELCOME TO EMA */}
        <div className={`rounded-xl shadow-xl border p-4 sm:p-6 mb-6 text-center card-with-dots ${
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
            <div className={`p-3 rounded-lg border-2 transition-all hover:scale-105 ${
              isDarkMode ? 'bg-blue-900/30 border-blue-600 hover:bg-blue-800/40' : 'bg-blue-50 border-blue-200 hover:bg-blue-100'
            }`}>
              <Search className="w-5 h-5 text-blue-500 mx-auto mb-2" />
              <p className={`text-xs font-semibold ${isDarkMode ? 'text-blue-300' : 'text-blue-700'}`}>Smart Search</p>
            </div>
            <div className={`p-3 rounded-lg border-2 transition-all hover:scale-105 ${
              isDarkMode ? 'bg-green-900/30 border-green-600 hover:bg-green-800/40' : 'bg-green-50 border-green-200 hover:bg-green-100'
            }`}>
              <Compass className="w-5 h-5 text-green-500 mx-auto mb-2" />
              <p className={`text-xs font-semibold ${isDarkMode ? 'text-green-300' : 'text-green-700'}`}>Navigation</p>
            </div>
            <div className={`p-3 rounded-lg border-2 transition-all hover:scale-105 ${
              isDarkMode ? 'bg-purple-900/30 border-purple-600 hover:bg-purple-800/40' : 'bg-purple-50 border-purple-200 hover:bg-purple-100'
            }`}>
              <Brain className="w-5 h-5 text-purple-500 mx-auto mb-2" />
              <p className={`text-xs font-semibold ${isDarkMode ? 'text-purple-300' : 'text-purple-700'}`}>AI Support</p>
            </div>
            <div className={`p-3 rounded-lg border-2 transition-all hover:scale-105 ${
              isDarkMode ? 'bg-orange-900/30 border-orange-600 hover:bg-orange-800/40' : 'bg-orange-50 border-orange-200 hover:bg-orange-100'
            }`}>
              <Shield className="w-5 h-5 text-orange-500 mx-auto mb-2" />
              <p className={`text-xs font-semibold ${isDarkMode ? 'text-orange-300' : 'text-orange-700'}`}>Trusted</p>
            </div>
          </div>
        </div>

        {/* Enhanced Search Section - Fully Responsive */}
        ...
        {/* Student Information - Fully Responsive */}
        {studentData && (
          <div className="space-y-6">
            {/* Exam Countdown - Responsive */}
            ...
            {/* Enhanced Basic Info - Responsive Grid */}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                <div className="space-y-4">
                  <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-700' : 'bg-gray-50'}`}>
                    <label className={`text-sm font-bold ${isDarkMode ? 'text-gray-300' : 'text-gray-500'} uppercase tracking-wide`}>
                      Full Name
                    </label>
                  ...
                  <div className={`p-4 rounded-lg ${isDarkMode ? 'bg-gray-700' : 'bg-gray-50'}`}>
                    ...
                    <p className={`text-lg font-bold ${isDarkMode ? 'text-white' : 'text-gray-900'} mt-1 font-mono`}>
                      {studentData.roll}
                    </p>
                  </div>
                </div>
            ...
            {/* Enhanced Exam Details - Responsive Cards */}
            <div className={`rounded-xl shadow-xl border p-4 sm:p-6 card-with-dots ${
              isDarkMode ? 'bg-gray-800/90 border-gray-700 card-with-dots-dark' : 'bg-white/90 border-gray-200'
            }`}>
              <div className="flex items-center space-x-4 mb-6">
            ...
            {/* COMBINED: Live Location Map & Step-by-Step Directions - Single Card */}
            <div className={`rounded-xl shadow-xl border p-4 sm:p-6 card-with-dots ${
              isDarkMode ? 'bg-gray-800/90 border-gray-700 card-with-dots-dark' : 'bg-white/90 border-gray-200'
            }`}>
              {/* Header Section */}
            ...
            {/* Enhanced What to Bring - Mobile Optimized */}
            <div className={`rounded-xl shadow-xl border p-4 sm:p-6 card-with-dots ${
              isDarkMode ? 'bg-gray-800/90 border-gray-700 card-with-dots-dark' : 'bg-white/90 border-gray-200'
            }`}>
              <div className="flex items-center space-x-4 mb-6">
            ...
            {/* Enhanced Motivation Section - Responsive */}
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {quote && (
                <div className={`rounded-xl shadow-xl border p-4 sm:p-6 card-with-dots ${
                  isDarkMode 
              ...
              {/* Ema Advice Section - Mobile Optimized */}
              {currentEmaAdvice && (
                <div className={`rounded-xl shadow-xl border p-4 sm:p-6 card-with-dots ${
                  isDarkMode 
                    ? 'bg-gradient-to-br from-purple-900/50 to-indigo-900/50 border-purple-700 card-with-dots-dark' 
                  ...
                  <div className={`p-3 rounded-lg border-2 ${
                    isDarkMode ? 'bg-purple-800/30 border-purple-600' : 'bg-white border-purple-200'
                  }`}>
            ...
            {/* Success Preparation Card - Mobile Optimized */}
            <div className={`rounded-xl shadow-xl border p-4 sm:p-6 text-center card-with-dots ${
              isDarkMode 
        ...
        {/* Enhanced Footer with Ema Assistant Card - Mobile Optimized */}
